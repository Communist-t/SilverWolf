...
## ⚠️ CRITICAL: How to Read PowerPoint Documents

**NEVER use the `read` tool directly on .pptx files!** The `read` tool only works with plain text files.

### ✅ CORRECT Workflow for Reading .pptx

**Step 1: Use markitdown (RECOMMENDED)**
...
**Step 2: For advanced needs (comments, notes, layouts, animations)**

Only use this when you need raw XML access:

```bash
# Unpack
python ooxml/scripts/unpack.py path-to-file.pptx output_directory
...
# Repack when done
python ooxml/scripts/pack.py output_directory path-to-file.pptx
```

### ❌ WRONG Approaches (DO NOT DO THIS)

1. ❌ `read` tool on .pptx file → Will return empty
2. ❌ Manual zipfile parsing → Unnecessary complexity
3. ❌ Writing custom Python scripts to parse XML → Use unpack.py instead

### 📋 Quick Decision Tree

```
Need to read .pptx?
├─ Just need text content? → markitdown (Step 1)
└─ Need comments/notes/layouts/XML? → Unpack (Step 2)
```
